...
void setup() {
  ...
  LINE.notify("Z10 Ready..."); 
}

void loop() {

  // MeOH Z10 3V3
  alarm_z10 = digitalRead(sensor8);
  if (alarm_z10 == 1){
  Serial.print("Z10: ");
  Serial.println(alarm_z10); 
  limit_time_z10 -= 1;
  if(limit_time_z10 == 0) {LINE.notify("Z10 - Internal room1"); LINE.notifyPicture("https://media.tenor.com/bH5qXUAuk4kAAAAC/fire.gif");}
  Serial.print("REPEAT ALARM: ");
  Serial.println(repeat_alarm);
  if(limit_time_z10/65 == repeat_alarm) {LINE.notify("(REPEAT) Z10 - Internal room1"); LINE.notifyPicture("https://media.tenor.com/bH5qXUAuk4kAAAAC/fire.gif"); repeat_alarm--;}
  }
